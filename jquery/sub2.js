...
$(document).ready(function() {
    ...
    // 초기 상태 설정
    $('.opencbox').css('left', '100%');

    // SlideLeft (열기 - Toggle)
    $('.openc').on('click', function() {
        const opencbox = $(this).closest('.doc').find('.opencbox');
        if (opencbox.css('display') === 'none' || opencbox.css('left') === '100%') {
            // 열기
            opencbox.css('display', 'block').animate({ left: '20%' }, 300);
        } else {
            // 닫기
            opencbox.animate({ left: '100%' }, 300, function() {
                opencbox.css('display', 'none');
            });
        }
    });

    // SlideRight (닫기)
    $('.opencbox').on('click', function() {
